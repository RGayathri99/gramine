 ...
 * unpoisoning the stack. When function return normally, they clean up after themselves, but we have
 * cases where a function never returns (thread exit, or change of stacks). So the requirements are:
 *
 ...
 *
 * - When a thread changes stacks, but we expect it to return to the same stack again, it should
 *   unpoison that stack.
 *
 *   TODO explain more
 *   TODO add an assertion that a stack is clean
 */
...
/*
 * Called when exiting a function marked as no-return. In the original AddressSanitizer, upoisons
 * the region starting from the current stack frame, until stack top.
 *
 * NOTE: Our implementation is a no-op, because it's inconvenient to determine the top of the
 * current stack in a universal fashion. Instead, we rely on Gramine to clean up the right stack.
 */
